...
- Imported 52 SVG face cards from `notpeter/Vector-Playing-Cards` into `assets/runtime/cards/faces/`.
- Added FishKiller runtime card back at `assets/runtime/cards/back.svg`.
- Added `assets/runtime/cards/card-manifest.json` mapping short card codes such as `Ah`, `Ks`, `Td`, and `7c` to SVG face files.
- Added `docs/third-party-assets.md` with source, imported commit, and README license excerpts.
- `src/render/fk2-table-scene.js` now loads the manifest with scene assets and draws hero/board cards from SVG textures when available.
- Primitive card rendering remains as fallback if the manifest or an individual texture fails to load.
- Hero cards remain anchored to each seat through `HERO_CARD_ANCHORS`; the current table and seat coordinate layout is preserved.

QA method:

- Automated screenshot capture was not used for this pass because the current working flow is to leave localhost running for manual screenshots and feedback.
- Code/server checks, manifest checks, asset-count checks, and coordinate-projection QA were run against the active Pixi frame at 1920 x 1080, 1440 x 900, and 1366 x 768.
...
- 1440 x 900: pass. All six hero-card anchors remain in-frame; 3-, 4-, and 5-card board states use the SVG texture path without moving the board slots.
...
1. Pixi now uses individual SVG textures, not a packed atlas; a future atlas may still be useful for batching and final packaging.
